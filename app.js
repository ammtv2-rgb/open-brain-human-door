...
function updateDashboard(rows) {
  ...
  let addedTodayEl = document.getElementById('recentlyAddedCount');

  // ---------- Inject "Completed This Week" ----------
  if (!weekCountEl) {
  ...
  if (!addedTodayEl) {
    ...
    if (weekCard && weekCard.parentNode) {
      ...
      addedCard.className = 'dashboard-filter-card';
      ...
      weekCard.parentNode.insertBefore(addedCard, weekCard.nextSibling);
      addedTodayEl = document.getElementById('recentlyAddedCount');
    }
  }
  ...
  if (addedTodayEl) addedTodayEl.textContent = recentlyAdded;
}
  injectCompletedTodayCard();
